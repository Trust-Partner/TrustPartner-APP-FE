...
export default function GeneralContractModal({ onBack }: Props) {
  const { formData, updateField, saveDraftData, step, nextStep, prevStep } =
    useContractForm('general');

  return (
    <Modal visible transparent animationType="fade">
      <View style={s.overlay}>
        <View style={s.container}>
          <Text style={s.title}>일반계약서 작성 (Step {step})</Text>
          ...
          <View style={s.footer}>
            ...
            {step < 2 ? (
              ...
              <TouchableOpacity
                ...
                onPress={() => console.log('제출', formData)}
              >
          ...
          <View style={s.footer}>
            <TouchableOpacity
              style={[s.btn, { backgroundColor: '#eee' }]}
            ...
            <TouchableOpacity
              style={[s.btn, { backgroundColor: '#ddd' }]}
              onPress={onBack}
            >
